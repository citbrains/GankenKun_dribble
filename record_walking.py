...
#from agent_dqn.model import Qnetwork
#from agent_dqn.policy import EpsilonGreedyPolicy
from util import now_str, RecordHistory

def train():
    # setup ===========================
    max_episode = 30  # 学習において繰り返す最大エピソード数
    ...
    memory_size = 10000
    batch_size = 32
    result_dir = os.path.join('./result/walk/dataset', now_str())
    x = []
    x_reward = []
    y_loss = []
    y_td_error = []
...
#    header = ["num_episode", "loss", "td_error", "reward_avg"]
    header = ["num_step", "x0", "y0", "sin0", "cos0", "x1", "y1", "sin1", "cos1","x2", "y2", "sin2", "cos2", "action", "x3", "y3", "sin3", "cos3"]
...
#    print('warming up {:,} steps... done.'.format(n_warmup_steps))
    ...
    while episode_loop:
        ...
        state_list, action_list, next_state_list = [[0.]*4 for i in range(3)], [], []
        state = env.reset()
...
#            action, epsilon, q_values = policy.get_action(state, actions_list)
            ...
            for i in range(len(actions_list)):
                action_num.append(i)
            action_rand = np.random.choice(action_num)
            action = actions_list[action_rand]
...
#            td_list.append(td_error)
            ...
            memory_state = state_list[-3:]

            history = {
                "num_step": total_step, 
                "x0": memory_state[0][0], 
                "y0": memory_state[0][1],
                "sin0": memory_state[0][2], 
                "cos0": memory_state[0][3], 
                "x1": memory_state[1][0], 
                "y1": memory_state[1][1],
                "sin1": memory_state[1][2], 
                "cos1": memory_state[1][3], 
                "x2": memory_state[2][0], 
                "y2": memory_state[2][1],
                "sin2": memory_state[2][2], 
                "cos2": memory_state[2][3], 
                "action": action,
                "x3": next_state[0], 
                "y3": next_state[1],
                "sin3": next_state[2], 
